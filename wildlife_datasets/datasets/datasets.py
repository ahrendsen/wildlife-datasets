...
class WildlifeDataset:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            not self.saved_to_system_folder
            and not root is None
            and not os.path.exists(root)
        ):
            raise Exception(
                "root does not exist. You may have have mispelled it."
            )
        ...
        if df is None:
            ...
            if not self.determined_by_df:
                print(
                    "This dataset is not determined by dataframe. But you construct it so."
                )
        if remove_unknown:
        ...
        self.labels, self.labels_map = pd.factorize(
            self.df[self.col_label].to_numpy()
        )
    ...
    def apply_segmentation(self, img: Image, idx: int) -> Image:
        ...
        if self.img_load in [
            ...
            if not ("segmentation" in data):
                raise ValueError(
                    f"{self.img_load} selected but no segmentation found."
                )
            segmentation = data["segmentation"]
            if isinstance(segmentation, list) or isinstance(
        ...
        if self.img_load in ["bbox"]:
            ...
            if not ("bbox" in data):
                raise ValueError(
                    f"{self.img_load} selected but no bbox found."
                )
    ...
    def download(cls, root: str, force: bool = False, **kwargs) -> None:
        ...
        if cls.saved_to_system_folder:
            ...
            if (
                ...
                with open(
                    os.path.join(root, cls.license_file_name), "w"
                ) as file:
    ...
    def fix_labels_replace_identity(
        ...
        """Replaces all instances of given identity with a new one.
    ...
    def fix_labels_remove_identity(
        ...
        """Removes all instances of given identities.
    ...
    def fix_labels_replace_images(
        ...
        for image_name, old_identity, new_identity in replace_identity:
            ...
            for index, df_row in df.iterrows():
                ...
                if (
                    image_name in df_row[self.col_path]
                    and old_identity == df_row[col]
                ):
                    df.loc[index, col] = new_identity
                    n_replaced += 1
            if n_replaced == 0:
                print(
                    "File name %s with identity %s was not found."
    ...
    def check_required_columns(self, df: pd.DataFrame = None) -> None:
        ...
        for col_name in ["image_id", self.col_label, self.col_path]:
            if col_name not in df.columns:
                raise (
                    Exception(
                        "Column %s must be in the dataframe columns."
                        % col_name
                    )
                )
    ...
    def check_types_column(
        self, col: pd.Series, col_name: str, allowed_types: List[str]
    ) -> None:
        ...
        if "list_numeric" in allowed_types and pd.api.types.is_list_like(col):
            ...
            for val in col:
                if not pd.api.types.is_list_like(
                    val
                ) and not pd.api.types.is_numeric_dtype(pd.Series(val)):
                    check = False
                    break
            if check:
                return None
        ...
        raise (
            Exception(
                "Column %s has wrong type. Allowed types = %s"
                % (col_name, str(allowed_types))
            )
        )

    def reorder_df(self, df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        **kwargs,
        ...
        if header_cols is not None:
            ...
            if len(header_cols) != n_cols:
                raise (
                    Exception(
                        "Length of header_cols must be the same as n_cols."
                    )
                )
        else:
            offset_h = 0
